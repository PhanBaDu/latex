...
\textbf{Sơ đồ nguyên lý và mạch thật} (thay cho sơ đồ đấu dây dạng text): Hình~\ref{fig:gpio-schematic} và Hình~\ref{fig:gpio-pcb}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{\FigSchA}
    \caption{Sơ đồ nguyên lý: NodeMCU ESP8266, DHT11, OLED (I$^2$C), LED và điện trở hạn dòng --- tương ứng bảng kết nối Bảng~\ref{tbl:gpio}.}
...
\begin{figure}[htbp]
    ...
    \caption{Ảnh mạch in thực tế (mặt hàn linh kiện): module nguồn hạ áp, các mối hàn tới ESP8266 và ngoại vi.}
    \label{fig:gpio-pcb}
\end{figure}
