...
\usepackage{amsmath} % phải trước newtxmath (không dùng amssymb — trùng \Bbbk với newtxmath)
...
\titleformat{name=\section,numberless}[block]
  {\normalfont\fontsize{15}{18}\selectfont\bfseries\centering}{}{0em}{}
...
% Ảnh minh họa mục kết quả — cùng \file{images/} với logo
\newcommand{\FigSchA}{11_A.jpg}
\newcommand{\FigSchB}{11_B.jpg}
...
\noindent
\begin{longtable}[l]{@{}p{3.0cm}p{4.4cm}p{7.4cm}@{}}
    \toprule
    \centering\textbf{Ký hiệu} & \centering\textbf{Tiếng Anh} & \centering\textbf{Tiếng Việt} \\
    \midrule\endhead
    \bottomrule\endfoot
    \textbf{API}   & Application Programming Interface   & Giao diện lập trình ứng dụng \\[0.3ex]
    \textbf{DHT}   & Digital Humidity and Temperature   & Cảm biến nhiệt độ \& độ ẩm số \\[0.3ex]
    \textbf{GPIO}  & General Purpose Input/Output       & Chân vào/ra đa mục đích \\[0.3ex]
    \textbf{HTTP}  & HyperText Transfer Protocol       & Giao thức truyền siêu văn bản \\[0.3ex]
    \textbf{I$^2$C} & Inter-Integrated Circuit          & Giao thức truyền thông hai dây \\[0.3ex]
    \textbf{JSON}  & JavaScript Object Notation        & Ký hiệu đối tượng JavaScript --- định dạng chuỗi dữ liệu \\[0.3ex]
    \textbf{LED}   & Light-Emitting Diode               & Điốt phát quang \\[0.3ex]
    \textbf{MQTT}  & Message Queuing Telemetry Transport & Giao thức truyền thông điệp theo mô hình pub/sub \\[0.3ex]
    \textbf{NRD}   & Node-RED Dashboard                 & Giao diện dashboard của Node-RED \\[0.3ex]
    \textbf{NTP}   & Network Time Protocol               & Giao thức đồng bộ thời gian qua mạng \\[0.3ex]
    \textbf{OLED}  & Organic Light-Emitting Diode        & Màn hình diod phát quang hữu cơ \\[0.3ex]
    \textbf{PWM}   & Pulse Width Modulation              & Điều chế độ rộng xung \\[0.3ex]
    \textbf{SDK}   & Software Development Kit            & Bộ công cụ phát triển phần mềm \\[0.3ex]
    \textbf{TLS}   & Transport Layer Security            & Giao thức bảo mật tầng truyền tải \\[0.3ex]
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        flowarrow/.style={-{Latex[length=2.4mm,width=1.7mm]}, semithick, draw=black!50, shorten >=0.6pt, shorten <=0.6pt},
        mqttn/.style={draw=black!42, line width=0.48pt, rounded corners=4pt, fill=green!10, inner sep=7pt, align=center, minimum width=3.15cm},
        fn/.style={draw=black!38, line width=0.45pt, rounded corners=4pt, fill=yellow!14, inner sep=6pt, align=center, text width=2.32cm, minimum height=0.95cm},
        uin/.style={draw=black!38, line width=0.45pt, rounded corners=4pt, fill=blue!10, inner sep=6pt, align=center, text width=2.5cm, minimum height=0.9cm},
    ]
        % --- Tọa độ cột (đủ rộng để 4 nhánh không chen) ---
        \def\xa{-5.25} \def\xb{-1.72} \def\xc{1.72} \def\xd{5.25}
        \node[mqttn] (m1) {\textbf{mqtt in}\\\texttt{iot/env/data}};
        \node[fn, text width=10.65cm, minimum height=1.2cm] (hd) at (0,-2.25) {\textbf{HandleData} (function)\\[3pt]parse JSON, tách 4 nhánh song song};
        \draw[flowarrow] (m1) -- (hd);
        \foreach \x/\lab in {-5.25/temperature, -1.72/humidity, 1.72/alert, 5.25/ledState}
            \node[fn] (\lab) at (\x,-4.65) {\textbf{\lab}};
        \coordinate (S) at (0,-3.12);
        \draw[flowarrow] (hd.south) -- (S);
        % Đường phân nhánh ngang mảnh (bus) — vẽ trước, mũi tên tới cột vẽ sau
        \begin{scope}[opacity=0.55]
            \draw[draw=black!28, line width=0.75pt, line cap=round] (-5.8,-3.12) -- (5.8,-3.12);
        \end{scope}
        \foreach \lab in {temperature, humidity, alert, ledState}
            \draw[flowarrow] (S) |- (\lab.north);
        \node[fn] (tf) at (\xa,-6.45) {temperature\\function};
        \node[fn] (hf) at (\xb,-6.45) {humidity\\function};
        \node[fn] (af) at (\xc,-6.45) {alert\\function};
        \node[fn] (lf) at (\xd,-6.45) {ledState\\function};
        \foreach \a/\b in {temperature/tf, humidity/hf, alert/af, ledState/lf}
            \draw[flowarrow] (\a) -- (\b);
        \node[uin] (u1) at (\xa,-8.65) {\textbf{ui\_gauge}\\\textbf{ui\_chart}};
        \node[uin] (u2) at (\xb,-8.65) {\textbf{ui\_gauge}\\\textbf{ui\_chart}};
        \node[uin] (u3) at (\xc,-8.65) {\textbf{ui\_toast}};
        \node[uin] (u4) at (\xd,-8.65) {\textbf{ui\_switch}};
        \foreach \a/\b in {tf/u1, hf/u2, af/u3, lf/u4}
            \draw[flowarrow] (\a) -- (\b);
    \end{tikzpicture}%
    }
    \caption{Luồng MQTT chính trong Node-RED}
    \label{fig:flow1}
\end{figure}
...
\begin{enumerate}[label=\arabic*., leftmargin=1.5cm]
    \item \textbf{Sơ đồ nguyên lý và mạch thật.} Kết nối theo firmware: DHT11 (DATA$\rightarrow$GPIO0 / D3), LED+R330 ($\rightarrow$GPIO12 / D6), OLED (SH1106) I$^2$C (SCL$\rightarrow$GPIO4 / D2, SDA$\rightarrow$GPIO5 / D1), 3V3 \& GND.

    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.92\textwidth]{\FigSchA}
        \caption{Sơ đồ nguyên lý (EasyEDA): NodeMCU ESP8266, DHT11, OLED (SH1106), LED qua điện trở.}
        \label{fig:res-sch}
    \end{figure}

    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.92\textwidth]{\FigSchB}
        \caption{Ảnh mạch thật đã lắp ráp (mặt linh kiện / PCB).}
        \label{fig:res-pcb}
    \end{figure}
...
% =============================================================================
\appendix
\section{Hướng dẫn nạp firmware cho ESP8266}
\label{sec:code}
\index{manguon}
...
% =============================================================================
\section{Hướng dẫn import Node-RED Flow}
\label{sec:flow}
\index{flow}
...
% =============================================================================
\section{Các nội dung khác (nếu có)}
\label{sec:phuluc-c}
\index{phuluckhac}
