...
\textbf{Ghi chú sơ đồ EasyEDA và firmware:} Trên Hình~\ref{fig:gpio-schematic} (mục~\ref{sec:gpio-easyeda}, file \file{11\_B.jpg}), OLED có thể được vẽ SCL$\rightarrow$D1 (GPIO5), SDA$\rightarrow$D2 (GPIO4) và điện trở LED 220$\Omega$ theo bản vẽ. Bảng~\ref{tbl:gpio} và mã Phụ lục A (\code{OLED\_SCL}=4, \code{OLED\_SDA}=5) mô tả đúng \textbf{cấu hình đang chạy} trên thiết bị; khi làm mạch mới cần thống nhất bản vẽ và \file{iot.txt}. Ảnh mạch thật: mục~\ref{sec:gpio-pcb-photo} (\file{11\_A.jpg}).
...
\begin{figure}[htbp]
    ...
    \caption{Sơ đồ nguyên lý vẽ trên EasyEDA (\file{11\_B.jpg}): NodeMCU ESP8266, DHT11, OLED (SH1106, I$^2$C), LED và điện trở hạn dòng.}
...
\begin{figure}[htbp]
    ...
    \caption{Ảnh mạch in thực tế (\file{11\_A.jpg}): mặt hàn linh kiện, module nguồn, mối hàn tới ESP8266 và ngoại vi.}
    \label{fig:gpio-pcb}
\end{figure}
...
\begin{enumerate}[label=\arabic*., leftmargin=1.5cm]
    \item \textbf{Sơ đồ nguyên lý và mạch thật.} Xem mục~\textbf{\ref{sec:ketnoi}} --- Hình~\ref{fig:gpio-schematic} và Hình~\ref{fig:gpio-pcb}. Tóm tắt kết nối theo firmware: DHT11 (DATA$\rightarrow$GPIO0 / D3), LED+R330 ($\rightarrow$GPIO12 / D6), OLED (SH1106) I$^2$C (SCL$\rightarrow$GPIO4 / D2, SDA$\rightarrow$GPIO5 / D1), 3V3 \& GND.
